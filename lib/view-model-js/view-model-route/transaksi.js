...
//update trans status after delivery
router.put("/updateTransStatus/:id_cabang/:id_transaksi", async (req, res) => {
  const id_cabang = req.params.id_cabang;
  const id_transaksi = Realm.BSON.ObjectId(req.params.id_transaksi);
  try {
    if (res.req.accepts("application/json")) {
      res.setHeader("Content-Type", "application/json");
    }
    const updatedTrans = await UpdateTransDeliveryStatus(id_transaksi);
...
router.post("/addDelivery/:id_cabang", async (req, res) => {
  ...
  try {
    ...
    redis.set("data_delivery_" + id_cabang, JSON.stringify(DeliveryAll));
  } catch (err) {
    console.error("Error inserting Transaction:", err);
    res.status(500).json({ message: "Internal Server Error" });
  }
...
router.put(
  "/updateDeliveryStatus/:id_cabang/:id_transaksi",
  async (req, res) => {
    const id_cabang = req.params.id_cabang;
    ...
    try {
      ...
      const updatedDelivery = await updatedeliveryStatus(id_transaksi);
